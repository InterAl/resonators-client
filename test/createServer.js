...
export default async function createServer(options = {}) {
    ...
    const serverPromise = readStdOutUntil(serverProcess, 'Node app is running', '[SERVER]', '\x1b[33m%s\x1b[0m:', options)
        .then(() => {
            console.log('server is ready');
        });

    const clientPromise = readStdOutUntil(clientProcess, 'webpack: Compiled', '[CLIENT]', '\x1b[36m%s\x1b[0m', options)
        .then(() => {
...
function readStdOutUntil(process, str, prefix, color, options) {
    return new Promise((res, rej) => {
        ...
        process.stdout.on('data', function(data) {
            ...
            options.showServerLogs && console.log(color, prefix, line);
        ...
        process.stderr.on('data', function(data) {
            options.showServerLogs && console.log(color, prefix, data.toString());
        });
    });
}
